namespace CarPartBotApi.Api.Logging;

public static partial class ApplicationLogs
{
    #region Exception handling middleware

    [LoggerMessage(EventId = 3_001_001, Level = LogLevel.Information, Message = "Request was cancelled.")]
    public static partial void RequestCancelled(this ILogger logger);

    [LoggerMessage(EventId = 3_001_002, Level = LogLevel.Error, Message = "Unhandled exception occurred.")]
    public static partial void UnhandledException(this ILogger logger, Exception exception);

    #endregion

    #region Webhook endpoints.

    [LoggerMessage(EventId = 3_002_001, Level = LogLevel.Warning, Message = "Webhook secret token is missing.")]
    public static partial void WebhookSecretTokenIsMissing(this ILogger logger);

    [LoggerMessage(EventId = 3_002_002, Level = LogLevel.Warning, Message = "Received webhook secret token is invalid.")]
    public static partial void WebhookSecretTokenIsInvalid(this ILogger logger);

    #endregion
}
